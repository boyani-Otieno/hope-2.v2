import React from 'react'
import './Leaders.css'
import img from '../assets/bish.jpeg'
import im3 from '../assets/vc.jpeg'
import im4 from '../assets/James.jpg'
import im5 from '../assets/Liz.jpeg'
import im6 from '../assets/maggy.jpeg'
import im7 from '../assets/peter.jpeg'
import im8 from '../assets/kelvin.jpeg'
import im9 from '../assets/Nion.jpeg'
import im10 from '../assets/Pst.Renson.jpeg'
import im11 from '../assets/Eunice.jpeg'
import im12 from '../assets/sly-sp.jpeg'
import { BsTwitterX } from "react-icons/bs";
import { BsFacebook, BsInstagram, BsLinkedin } from "react-icons/bs";

export default function Leaders() {
  return (
    <div className='leader'>
      <div className='lead-content'>
        <div className='row'>

         <div className='col-md-12 text-center'>
            <h4 className='lead-title'>
              The Team Behind Tumaini Ministries
            </h4>
            <p className='lead-subtitle'>
              Tumaini Ministries is led by a Bishop, Fred Ogutu Faradays and an Executive Board comprising 
              currently of 11 members all who are from various Tumaini Churches as members and leaders.
            </p>
         </div>
        
         <div className='col-sm-6 col-md-4'>
          <div className='lead-item'>
           <img className='lead-img'  src={img} alt='pic' />
           <h6>Fred Faradays Ogutu</h6>
           <div className='lead-info'>
            <p>Executive Board Chairman</p>
            <p>Fred is the current Bishop of Tumaini Ministries and the sole leader.</p>
              <ul className='lead-icons'>
                 <li><a href="https://twitter.com" target="_blank" rel='nooopener noreferrer'><BsTwitterX/></a>
                 </li>

                 <li><a href="https://facebook.com" target="_blank" rel='nooopener noreferrer'><BsFacebook/></a>
                 </li>

                 <li><a href="https://instagram.com" target="_blank" rel='nooopener noreferrer'><BsInstagram/></a>
                 </li>

                 <li><a href="https://facebook.com" target="_blank" rel='nooopener noreferrer'><BsLinkedin/></a>
                 </li>
              </ul>
           </div>
          </div>
          </div>


          <div className='col-sm-6 col-md-4'>
          <div className='lead-item'>
           <img className='lead-img'  src={im3} alt='pic' />
           <h6>Pastor James Mburu</h6>
           <div className='lead-info'>
            <p>Executive Board Vice Chairman</p>
            <p>Pastor Mburu is the Lead Pastor of Tumaini Gitwamba. He is the Vice Chairman hence assisting the Bishop In administration.
               He is also leader for other sub commitees.
            </p>
              <ul className='lead-icons'>
              <li><a href="https://twitter.com" target="_blank" rel='nooopener noreferrer'><BsTwitterX/></a>
                 </li>

                 <li><a href="https://facebook.com" target="_blank" rel='nooopener noreferrer'><BsFacebook/></a>
                 </li>

                 <li><a href="https://instagram.com" target="_blank" rel='nooopener noreferrer'><BsInstagram/></a>
                 </li>

                 <li><a href="https://facebook.com" target="_blank" rel='nooopener noreferrer'><BsLinkedin/></a>
                 </li>
              </ul>
           </div>
          </div>
          </div>
          <div className='col-sm-6 col-md-4'>
          <div className='lead-item'>
           <img className='lead-img'  src={im4} alt='pic' />
           <h6>Pastor James Wanyama</h6>
           <div className='lead-info'>
            <p>Executive Board Secretary</p>
            <p>Pastor James is a pastor at Tumaini Raila. He is the general Secretary of the Board assisted with Margaret.</p>
              <ul className='lead-icons'>
              <li><a href="https://twitter.com" target="_blank" rel='nooopener noreferrer'><BsTwitterX/></a>
                 </li>

                 <li><a href="https://facebook.com" target="_blank" rel='nooopener noreferrer'><BsFacebook/></a>
                 </li>

                 <li><a href="https://instagram.com" target="_blank" rel='nooopener noreferrer'><BsInstagram/></a>
                 </li>

                 <li><a href="https://facebook.com" target="_blank" rel='nooopener noreferrer'><BsLinkedin/></a>
                 </li>
              </ul>
           </div>
          </div>
          </div>
        
         <div className='col-sm-6 col-md-4'>
          <div className='lead-item'>
           <img className='lead-img'  src={im8} alt='pic' />
           <h6>Kelvin Otieno</h6>
           <div className='lead-info'>
            <p>Executive Board Tresurer</p>
            <p>Kelvin is a member and Worship Lead of Tumaini Gitwamba and the Ministry Treasurer. He
               works hand in hand with Peter.
            </p>
              <ul className='lead-icons'>
                 <li><a href="https://twitter.com" target="_blank" rel='nooopener noreferrer'><BsTwitterX/></a>
                 </li>

                 <li><a href="https://facebook.com" target="_blank" rel='nooopener noreferrer'><BsFacebook/></a>
                 </li>

                 <li><a href="https://instagram.com" target="_blank" rel='nooopener noreferrer'><BsInstagram/></a>
                 </li>

                 <li><a href="https://facebook.com" target="_blank" rel='nooopener noreferrer'><BsLinkedin/></a>
                 </li>
              </ul>
           </div>
          </div>
          </div>


          <div className='col-sm-6 col-md-4'>
          <div className='lead-item'>
           <img className='lead-img'  src={im11} alt='pic' />
           <h6>Eunice Atieno</h6>
           <div className='lead-info'>
            <p>Women's Ministry Lead</p>
            <p>Eunice is a member of Tumaini Kayaba and the Women's Ministry Lead.</p>
              <ul className='lead-icons'>
              <li><a href="https://twitter.com" target="_blank" rel='nooopener noreferrer'><BsTwitterX/></a>
                 </li>

                 <li><a href="https://facebook.com" target="_blank" rel='nooopener noreferrer'><BsFacebook/></a>
                 </li>

                 <li><a href="https://instagram.com" target="_blank" rel='nooopener noreferrer'><BsInstagram/></a>
                 </li>

                 <li><a href="https://facebook.com" target="_blank" rel='nooopener noreferrer'><BsLinkedin/></a>
                 </li>
              </ul>
           </div>
          </div>
          </div>
          <div className='col-sm-6 col-md-4'>
          <div className='lead-item'>
           <img className='lead-img'  src={im10} alt='pic' />
           <h6>Pastor Renson Mwembe</h6>
           <div className='lead-info'>
            <p>Pastor's Representative</p>
            <p>Pastor Renson is the Lead Pastor of Tumaini Kiambio and also the one representing all Pastors of Tumaini 
               Ministry Churches. He is also the Chairman of the Pastors Council.</p>
              <ul className='lead-icons'>
              <li><a href="https://twitter.com" target="_blank" rel='nooopener noreferrer'><BsTwitterX/></a>
                 </li>

                 <li><a href="https://facebook.com" target="_blank" rel='nooopener noreferrer'><BsFacebook/></a>
                 </li>

                 <li><a href="https://instagram.com" target="_blank" rel='nooopener noreferrer'><BsInstagram/></a>
                 </li>

                 <li><a href="https://facebook.com" target="_blank" rel='nooopener noreferrer'><BsLinkedin/></a>
                 </li>
              </ul>
           </div>
          </div>
          </div>

        
         <div className='col-sm-6 col-md-4'>
          <div className='lead-item'>
           <img className='lead-img'  src={im5} alt='pic' />
           <h6>Pastor Liz Pronk</h6>
           <div className='lead-info'>
            <p>Children Ministry Lead</p>
            <p>Pastor Liz is a member of Tumaini Grogon Church and also the children Ministry Lead, Leading all Children
              (Sunday Schools) in all Tumaini Churches.
            </p>
              <ul className='lead-icons'>
                 <li><a href="https://twitter.com" target="_blank" rel='nooopener noreferrer'><BsTwitterX/></a>
                 </li>

                 <li><a href="https://facebook.com" target="_blank" rel='nooopener noreferrer'><BsFacebook/></a>
                 </li>

                 <li><a href="https://instagram.com" target="_blank" rel='nooopener noreferrer'><BsInstagram/></a>
                 </li>

                 <li><a href="https://facebook.com" target="_blank" rel='nooopener noreferrer'><BsLinkedin/></a>
                 </li>
              </ul>
           </div>
          </div>
          </div>


          <div className='col-sm-6 col-md-4'>
          <div className='lead-item'>
           <img className='lead-img'  src={im7} alt='pic' />
           <h6>Pastor Peter Njasi</h6>
           <div className='lead-info'>
            <p>Ministry Accountant</p>
            <p>Pastor Peter is the Ministry accountant and he has been serving in Tumaini Ministry.
      
            </p>
              <ul className='lead-icons'>
              <li><a href="https://twitter.com" target="_blank" rel='nooopener noreferrer'><BsTwitterX/></a>
                 </li>

                 <li><a href="https://facebook.com" target="_blank" rel='nooopener noreferrer'><BsFacebook/></a>
                 </li>

                 <li><a href="https://instagram.com" target="_blank" rel='nooopener noreferrer'><BsInstagram/></a>
                 </li>

                 <li><a href="https://facebook.com" target="_blank" rel='nooopener noreferrer'><BsLinkedin/></a>
                 </li>
              </ul>
           </div>
          </div>
          </div>
          <div className='col-sm-6 col-md-4'>
          <div className='lead-item'>
           <img className='lead-img'  src={im6} alt='pic' />
           <h6>Margaret Wanjiku</h6>
           <div className='lead-info'>
            <p>Youth Lead & Ass. Secretary</p>
            <p>Margaret is a Member of Tumaini Grogon, the Mother church(First Church).</p>
              <ul className='lead-icons'>
              <li><a href="https://twitter.com" target="_blank" rel='nooopener noreferrer'><BsTwitterX/></a>
                 </li>

                 <li><a href="https://facebook.com" target="_blank" rel='nooopener noreferrer'><BsFacebook/></a>
                 </li>

                 <li><a href="https://instagram.com" target="_blank" rel='nooopener noreferrer'><BsInstagram/></a>
                 </li>

                 <li><a href="https://facebook.com" target="_blank" rel='nooopener noreferrer'><BsLinkedin/></a>
                 </li>
              </ul>
           </div>
          </div>
          </div>
        
         <div className='col-sm-6 col-md-4'>
          <div className='lead-item'>
           <img className='lead-img'  src={im9} alt='pic' />
           <h6>Nixon Mambo</h6>
           <div className='lead-info'>
            <p>Men's Ministry Lead</p>
            <p>Nixon Mambo is a member of Tumaini Fuata Nyayo church. He leads the Ministry of men incoorparting all men 
               from all Tumaini Churches.
            </p>
              <ul className='lead-icons'>
                 <li><a href="https://twitter.com" target="_blank" rel='nooopener noreferrer'><BsTwitterX/></a>
                 </li>

                 <li><a href="https://facebook.com" target="_blank" rel='nooopener noreferrer'><BsFacebook/></a>
                 </li>

                 <li><a href="https://instagram.com" target="_blank" rel='nooopener noreferrer'><BsInstagram/></a>
                 </li>

                 <li><a href="https://facebook.com" target="_blank" rel='nooopener noreferrer'><BsLinkedin/></a>
                 </li>
              </ul>
           </div>
          </div>
          </div>


          <div className='col-sm-6 col-md-4'>
          <div className='lead-item'>
           <img className='lead-img'  src={im12} alt='pic' />
           <h6>Sylvia Boyani Otieno</h6>
           <div className='lead-info'>
            <p>Communications & Media Personell</p>
            <p>Sylvia is the website developer and our main administrator of the website. She also handles all the
               ministry's Communications and media and all IT related matters.
            </p>
              <ul className='lead-icons'>
              <li><a href="https://twitter.com/BoyaniSylvia" target="_blank" rel='nooopener noreferrer'><BsTwitterX/></a>
                 </li>

                 <li><a href="https://facebook.com/Boyani Ombogo" target="_blank" rel='nooopener noreferrer'><BsFacebook/></a>
                 </li>

                 <li><a href="https://www.instagram.com/boyani.sylvia/" target="_blank" rel='nooopener noreferrer'><BsInstagram/></a>
                 </li>
                 <li><a href="https://ke.linkedin.com/in/sylvia-boyani-7130a6196" target="_blank" rel='nooopener noreferrer'><BsLinkedin/></a>
                 </li>
              </ul>
           </div>
          </div>
          </div>

        </div>
      </div>
    </div>
  )
}
